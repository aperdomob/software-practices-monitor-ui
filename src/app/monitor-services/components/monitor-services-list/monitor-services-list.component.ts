...
export class MonitorServicesListComponent implements OnInit {
  ...
  setIntegrationActiveById(id) {
    this.integrationsList.map(
      (integration) =>
        (integration.active = id === integration.id ? true : false)
    );
    this.integrationSelected = this.integrationsList.filter(
      (integration) => integration.active
    )[0];

    if (this.monitorServicesDetailComponent) {
      this.monitorServicesDetailComponent.changeIntegration(
